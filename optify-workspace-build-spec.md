...
> 📜 **역사 문서 (2026-07 초기 명세)**. 당시 계획을 그대로 남긴 것이라 현재 화면(파란 테마, 두 블록 사이드바, 영업·회계·팀 축, 트래커 연동)과 다릅니다.
> **현재 구조·권한·DB 적용 상태의 기준은 `CLAUDE.md`** 이고, DB 셋업은 `supabase/README.md` 를 봅니다.
