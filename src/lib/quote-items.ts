/**
 * 견적 품목 카탈로그 (config 기반 — nav.ts·channels.ts와 동일 패턴).
 * 2026-09-06: 견적서 화면의 카탈로그 선택 UI는 제거됨 — 품목은 손으로 입력한다.
 * 이 목록은 SEO 진단 → 견적 초안 프리필(CATALOG_ITEM_INDEX)과 단위 목록(QUOTE_UNITS)에만 쓴다.
 * 카탈로그 수정이 과거 견적에 영향 없음(견적은 jsonb 스냅샷).
 *
 * 기준단가(basePrice): 진단 프리필 시 들어가는 정가. 견적별 수동 조정 가능.
