...
서치콘솔·GA4 권한(2026-09-23): 조회는 `GOOGLE_SERVICE_ACCOUNT_KEY` 의 서비스 계정으로 하므로 고객사 서치콘솔(사용자 및 권한)·GA4(속성 액세스 관리)에 그 계정을 사용자로 추가해야 한다. `/api/google/service-account`(GET, 팀원)가 이메일을 주고 서치콘솔·GA4 화면이 안내·복사 버튼으로 보여 준다. 403/PERMISSION_DENIED 는 `friendlyGoogleError` 로 사람 말로. URL 접두 속성은 `normalizeSiteUrl` 이 끝에 / 를 붙인다(없으면 403).
